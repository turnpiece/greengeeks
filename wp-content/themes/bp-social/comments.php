...
<?php if ( $user_ID ) : ?>
<?php
    $profile_url = get_site_url().'/wp-admin/profile.php';
    global $bp;
    if($bp) $profile_url = bp_core_get_user_domain( $user_ID );
?>
<label>
    <?php printf(__('Logged in as %s.',TEMPLATE_DOMAIN), '<a href="'.$profile_url.'">'.$user_identity.'</a>'); ?>
    <?php $mywp_version = get_bloginfo('version'); if ($mywp_version >= '2.7') { ?>
        <a href="<?php echo wp_logout_url( get_site_url() ); ?>">
            <?php _e('Log out &raquo;',TEMPLATE_DOMAIN); ?>
        </a>
    <?php } else { ?>
        <a href="<?php echo site_url(); ?>/wp-login.php?action=logout" title="<?php __('Log out of this account',TEMPLATE_DOMAIN); ?>">
            <?php _e('Log out &raquo;',TEMPLATE_DOMAIN); ?>
        </a>
    <?php } ?>
</label>

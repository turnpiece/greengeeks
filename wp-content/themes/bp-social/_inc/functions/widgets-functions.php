	...
	<!--//--><![CDATA[//><!--
	    ...
	    function twitterCallback2(twitters) {
	      ...
	      for (var i=0; i<twitters.length; i++){
	        ...
	        statusHTML.push('<li><span class="">'+status+'</span> <a style="font-size:85%" class="time" href="http://twitter.com/'+username+'/statuses/'+twitters[i].id_str+'">'+relative_time(twitters[i].created_at)+'</a></li>');
	      }
...
if(isset($instance['title']))$twitter_title = $instance['title']; else $twitter_title = '';
if(isset($instance['twitter_count']))$twitter_count = $instance['twitter_count']; else $twitter_count = '';
?>
...
<?php echo __('Twitter User Name:', TEMPLATE_DOMAIN)?></label>
...
if(isset($instance['number']) && $instance['number'] != '')$mc_number = $instance['number']; else $mc_number = '0,2';
...
$mostcommenteds = $wpdb->get_results("SELECT $wpdb->posts.ID, post_title, post_name, post_date, COUNT($wpdb->comments.comment_post_ID) AS 'comment_total' FROM $wpdb->posts LEFT JOIN $wpdb->comments ON $wpdb->posts.ID = $wpdb->comments.comment_post_ID WHERE comment_approved = '1' AND post_date_gmt < '" . gmdate("Y-m-d H:i:s") . "' AND post_status = 'publish' AND post_password = '' GROUP BY $wpdb->comments.comment_post_ID ORDER  BY comment_total DESC LIMIT $mc_number");
  ...
  foreach ($mostcommenteds as $posts) {
    $post_title = htmlspecialchars(stripslashes($posts->post_title));
    $comment_total = (int) $posts->comment_total;
...
function form($instance) {
// Get the options into variables, escaping html characters on the way
if(isset($instance['title']))$mc_name = $instance['title']; else $mc_name = '';
if(isset($instance['number']))$mc_number = $instance['number']; else $mc_number = '';
if(isset($instance['commentcount']))$mc_comment_count = $instance['commentcount']; else $mc_comment_count = '';
?>
<p>
...
if(isset($instance['number']) && $instance['number'] != '')$rc_number = $instance['number']; else $rc_number = '0,2';
...
if(isset($comment->comment_author_name))$grav_name = $comment->comment_author_name; else $grav_name = '';
...
<?php if(isset($bp_existed) && $bp_existed == 'true') { ?>
...
function form($instance) {
// Get the options into variables, escaping html characters on the way
if(isset($instance['title']))$rc_name = $instance['title']; else $rc_name = '';
if(isset($instance['number']))$rc_number = $instance['number']; else $rc_number = '';
if(isset($instance['avatar_on']))$rc_avatar = $instance['avatar_on']; else $rc_avatar = '';
?>
...
function form($instance) {
// Get the options into variables, escaping html characters on the way
if(isset($instance['title']))$fli_name = $instance['title']; else $fli_name = '';
if(isset($instance['id']))$fli_id = $instance['id']; else $fli_id = '';
if(isset($instance['number']))$fli_number = $instance['number']; else $fli_number = '';
?>
...
function form($instance) {
// Get the options into variables, escaping html characters on the way
if(isset($instance['title']))$feat_title = $instance['title']; else $feat_title = '';
if(isset($instance['featcatname']))$feat_name = $instance['featcatname']; else $feat_name = '';
if(isset($instance['featthumb']))$feat_thumb = $instance['featthumb']; else $feat_thumb = '';
if(isset($instance['feattotal']))$feat_total = $instance['feattotal']; else $feat_total = '';
?>
...
function form($instance) {
// Get the options into variables, escaping html characters on the way
if(isset($instance['title'])) $bps_name = $instance['title']; else $bps_name = '';
?>
<p>
...
function form($instance) {
// Get the options into variables, escaping html characters on the way
if(isset($instance['title'])) $rg_name = $instance['title']; else $rg_name = '';
?>
<p>
<label for="<?php echo $this->get_field_id('title'); ?>"><?php  _e('Name',TEMPLATE_DOMAIN); ?>:</label>

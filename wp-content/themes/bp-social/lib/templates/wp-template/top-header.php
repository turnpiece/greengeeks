...
<?php
if($tn_buddysocial_video_code != "") {
	if ( preg_match('/^http/i', $tn_buddysocial_video_code) )
		$video_url = $tn_buddysocial_video_code;
	else
		$video_url = 'http://youtu.be/'.$tn_buddysocial_video_code; // Backward compatibility with Youtube video code
	if ( $video_url == get_transient('tn_buddysocial_video_url') )
		$video_code = get_transient('tn_buddysocial_video_code');
	if ( !isset($video_code) || !$video_code ){
		$video_code = wp_oembed_get($video_url, array('width' => 480));
		set_transient('tn_buddysocial_video_url', $video_url);
		set_transient('tn_buddysocial_video_code', $video_code);
	}
?>

...
$theme_data = wp_get_theme( 'bp-community' );
} else {
...
<?php $vc = 2; $i = 0; foreach ($value_var_global as $value_var) { ?>
<ul style="" class="ui-tabs-panel<?php if($vc > 0) { ?> ui-tabs-hide<?php } ?>" id="tab<?php echo $vc; ?>">
<li>
<?php foreach ($options as $value) { ?>

<?php if (($value['inblock'] == $value_var) && (isset($value['header-title']) && $value['header-title'] != "")) { // if we got header title for option ?>
...
<div class="description"><?php echo $value['name']; ?><br /><span><?php echo isset($value['description']) ? $value['description'] : ''; ?></span></div>
...
$post_bg_color = isset($_POST[ 'tn_buddycom_adminbar_bg_color' ]) ? $_POST[ 'tn_buddycom_adminbar_bg_color' ] : '';
$post_hover_bg_color = isset($_POST[ 'tn_buddycom_adminbar_hover_bg_color' ]) ? $_POST[ 'tn_buddycom_adminbar_hover_bg_color' ] : '';

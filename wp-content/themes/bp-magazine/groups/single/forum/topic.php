...
<?php if ( bp_has_forum_topic_posts() ) : ?>
	...
	<form action="<?php bp_forum_topic_action() ?>" method="post" id="forum-topic-form" class="standard-form">
		...
		<div class="item-list-tabs no-ajax" id="subnav" role="navigation">
			<ul>
				<li>
					<a href="<?php bp_forum_topic_new_reply_link() ?>" class="new-reply-link"><?php _e( 'New Reply', 'bp_magazine' ) ?></a>
				</li>

				<?php if ( bp_forums_has_directory() ) : ?>

					<li>
						<a href="<?php bp_forums_directory_permalink() ?>"><?php _e( 'Forum Directory', 'bp_magazine') ?></a>
					</li>
		...
		<div id="topic-meta">
			...
			<?php if ( bp_forum_topic_has_tags() ) : ?>
				...
				<div class="topic-tags">
					...
					<?php _e( 'Topic tags:', 'bp_magazine' ) ?> <?php bp_forum_topic_tag_list() ?>
			...
			<?php while ( bp_forum_topic_posts() ) : bp_the_forum_topic_post(); ?>
				...
				<li id="post-<?php bp_the_topic_post_id() ?>" class="<?php bp_the_topic_post_css_class() ?>">
					<div class="poster-meta">
						...
						<?php echo sprintf( __( '%1$s said %2$s:', 'bp_magazine' ), bp_get_the_topic_post_poster_name(), bp_get_the_topic_post_time_since() ) ?>
					</div>
					...
					<div class="admin-links">
						...
						<a href="#post-<?php bp_the_topic_post_id() ?>" title="<?php _e( 'Permanent link to this post', 'bp_magazine' ) ?>">#</a>
					</div>
				</li>
			...
			<?php if ( bp_get_the_topic_is_last_page() ) : ?>
				...
				<?php if ( bp_get_the_topic_is_topic_open() && !bp_group_is_user_banned() ) : ?>
					...
					<div id="post-topic-reply">
						...
						<?php if ( bp_groups_auto_join() && !bp_group_is_member() ) : ?>
							<p><?php _e( 'You will auto join this group when you reply to this topic.', 'bp_magazine' ) ?></p>
						<?php endif; ?>

						<?php do_action( 'groups_forum_new_reply_before' ) ?>

						<h4><?php _e( 'Add a reply:', 'bp_magazine' ) ?></h4>

						<textarea name="reply_text" id="reply_text"></textarea>

						<div class="submit">
							<input type="submit" name="submit_reply" id="submit" value="<?php _e( 'Post Reply', 'bp_magazine' ) ?>" />
						</div>
				...
				<?php elseif ( !bp_group_is_user_banned() ) : ?>
					...
					<div id="message" class="info">
						<p><?php _e( 'This topic is closed, replies are no longer accepted.', 'bp_magazine' ) ?></p>
					</div>
...
<?php else: ?>
	...
	<div id="message" class="info">
		<p><?php _e( 'There are no posts for this topic.', 'bp_magazine' ) ?></p>
	</div>

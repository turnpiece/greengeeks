 ...
 */
function bp_dtheme_ajax_querystring_magazine( $query_string, $object ) {
	global $bp;
	...
	return apply_filters( 'bp_dtheme_ajax_querystring_magazine', $query_string, $object, $object_filter, $object_scope, $object_page, $object_search_terms, $object_extras );
}
add_filter( 'bp_ajax_querystring', 'bp_dtheme_ajax_querystring_magazine', 10, 2 );

/* This function will simply load the template loop for the current object. On an AJAX request */
function bp_dtheme_object_template_loader_magazine() {
...
add_action( 'wp_ajax_members_filter', 'bp_dtheme_object_template_loader_magazine' );
add_action( 'wp_ajax_groups_filter',  'bp_dtheme_object_template_loader_magazine' );
add_action( 'wp_ajax_blogs_filter',   'bp_dtheme_object_template_loader_magazine' );
add_action( 'wp_ajax_forums_filter',  'bp_dtheme_object_template_loader_magazine' );

// This function will load the activity loop template when activity is requested via AJAX
function bp_dtheme_activity_template_loader_magazine() {
	global $bp;
...
add_action( 'wp_ajax_activity_widget_filter', 'bp_dtheme_activity_template_loader_magazine' );
add_action( 'wp_ajax_activity_get_older_updates', 'bp_dtheme_activity_template_loader_magazine' );

/* AJAX update posting */
function bp_dtheme_post_update_magazine() {
	global $bp;
	...
	if ( empty( $_POST['content'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'Please enter some content to post.', 'bp_magazine' ) . '</p></div>';
		return false;
	}
	...
	if ( empty( $activity_id ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was a problem posting your update, please try again.', 'bp_magazine' ) . '</p></div>';
		return false;
	}
...
add_action( 'wp_ajax_post_update', 'bp_dtheme_post_update_magazine' );

/* AJAX activity comment posting */
function bp_dtheme_new_activity_comment_magazine() {
	global $bp;
	...
	if ( empty( $_POST['content'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'Please do not leave the comment area blank.', 'bp_magazine' ) . '</p></div>';
		return false;
	}

	if ( empty( $_POST['form_id'] ) || empty( $_POST['comment_id'] ) || !is_numeric( $_POST['form_id'] ) || !is_numeric( $_POST['comment_id'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was an error posting that reply, please try again.', 'bp_magazine' ) . '</p></div>';
		return false;
	}
...
add_action( 'wp_ajax_new_activity_comment', 'bp_dtheme_new_activity_comment_magazine' );

/* AJAX delete an activity */
function bp_dtheme_delete_activity_magazine() {
	global $bp;
	...
	if ( !bp_activity_delete( array( 'id' => $activity->id, 'user_id' => $activity->user_id ) ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was a problem when deleting. Please try again.', 'bp_magazine' ) . '</p></div>';
		return false;
	}
...
add_action( 'wp_ajax_delete_activity', 'bp_dtheme_delete_activity_magazine' );

/* AJAX delete an activity comment */
function bp_dtheme_delete_activity_comment_magazine() {
	global $bp;
...
add_action( 'wp_ajax_delete_activity_comment', 'bp_dtheme_delete_activity_comment_magazine' );

/* AJAX mark an activity as a favorite */
function bp_dtheme_mark_activity_favorite_magazine() {
	global $bp;

	bp_activity_add_user_favorite( $_POST['id'] );
	_e( 'Remove Favorite', 'bp_magazine' );
}
add_action( 'wp_ajax_activity_mark_fav', 'bp_dtheme_mark_activity_favorite_magazine' );

/* AJAX mark an activity as not a favorite */
function bp_dtheme_unmark_activity_favorite_magazine() {
	global $bp;

	bp_activity_remove_user_favorite( $_POST['id'] );
	_e( 'Favorite', 'bp_magazine' );
}
add_action( 'wp_ajax_activity_mark_unfav', 'bp_dtheme_unmark_activity_favorite_magazine' );
 ...
 * @since 1.5
 */
function bp_dtheme_get_single_activity_content_magazine() {
	...
	do_action_ref_array( 'bp_dtheme_get_single_activity_content_magazine', array( &$activity ) );
...
add_action( 'wp_ajax_get_single_activity_content', 'bp_dtheme_get_single_activity_content_magazine' );

/* AJAX invite a friend to a group functionality */
function bp_dtheme_ajax_invite_user_magazine() {
	global $bp;
	...
	if ( 'invite' == $_POST['friend_action'] ) {
		...
		echo '<div class="action">
				<a class="button remove" href="' . wp_nonce_url( $bp->loggedin_user->domain . bp_get_groups_slug() . '/' . $_POST['group_id'] . '/invites/remove/' . $user->id, 'groups_invite_uninvite_user' ) . '" id="uid-' . esc_attr( $user->id ) . '">' . __( 'Remove Invite', 'bp_magazine' ) . '</a>
			  </div>';
		echo '</li>';
...
add_action( 'wp_ajax_groups_invite_user', 'bp_dtheme_ajax_invite_user_magazine' );

/* AJAX add/remove a user as a friend when clicking the button */
function bp_dtheme_ajax_addremove_friend_magazine() {
	global $bp;
	...
	if ( 'is_friend' == BP_Friends_Friendship::check_is_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
		...
		if ( !friends_remove_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
			echo __("Friendship could not be canceled.", 'bp_magazine');
		} else {
			echo '<a id="friend-' . $_POST['fid'] . '" class="add" rel="add" title="' . __( 'Add Friend', 'bp_magazine' ) . '" href="' . wp_nonce_url( $bp->loggedin_user->domain . bp_get_friends_slug() . '/add-friend/' . $_POST['fid'], 'friends_add_friend' ) . '">' . __( 'Add Friend', 'bp_magazine' ) . '</a>';
		}
		...
		if ( !friends_add_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
			echo __("Friendship could not be requested.", 'bp_magazine');
		} else {
			echo '<a href="' . $bp->loggedin_user->domain . bp_get_friends_slug() . '/requests" class="requested">' . __( 'Friendship Requested', 'bp_magazine' ) . '</a>';
		}
	} else {
		echo __( 'Request Pending', 'bp_magazine' );
	}

	return false;
}
add_action( 'wp_ajax_addremove_friend', 'bp_dtheme_ajax_addremove_friend_magazine' );

/* AJAX accept a user as a friend when clicking the "accept" button */
function bp_dtheme_ajax_accept_friendship_magazine() {
	check_admin_referer( 'friends_accept_friendship' );

	if ( !friends_accept_friendship( $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem accepting that request. Please try again.', 'bp_magazine' ) . '</p></div>';

	return true;
}
add_action( 'wp_ajax_accept_friendship', 'bp_dtheme_ajax_accept_friendship_magazine' );

/* AJAX reject a user as a friend when clicking the "reject" button */
function bp_dtheme_ajax_reject_friendship_magazine() {
	check_admin_referer( 'friends_reject_friendship' );

	if ( !friends_reject_friendship( $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem rejecting that request. Please try again.', 'bp_magazine' ) . '</p></div>';

	return true;
}
add_action( 'wp_ajax_reject_friendship', 'bp_dtheme_ajax_reject_friendship_magazine' );

/* AJAX join or leave a group when clicking the "join/leave" button */
function bp_dtheme_ajax_joinleave_group_magazine() {
	global $bp;
	...
	if ( !groups_is_user_member( $bp->loggedin_user->id, $group->id ) ) {
		...
		if ( 'public' == $group->status ) {
			...
			if ( !groups_join_group( $group->id ) ) {
				_e( 'Error joining group', 'bp_magazine' );
			} else {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="leave-group" rel="leave" title="' . __( 'Leave Group', 'bp_magazine' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'leave-group', 'groups_leave_group' ) . '">' . __( 'Leave Group', 'bp_magazine' ) . '</a>';
			}
			...
			if ( !groups_send_membership_request( $bp->loggedin_user->id, $group->id ) ) {
				_e( 'Error requesting membership', 'bp_magazine' );
			} else {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="membership-requested" rel="membership-requested" title="' . __( 'Membership Requested', 'bp_magazine' ) . '" href="' . bp_get_group_permalink( $group ) . '">' . __( 'Membership Requested', 'bp_magazine' ) . '</a>';
			}
		}
		...
		if ( !groups_leave_group( $group->id ) ) {
			_e( 'Error leaving group', 'bp_magazine' );
		} else {
			if ( 'public' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="join-group" rel="join" title="' . __( 'Join Group', 'bp_magazine' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'join', 'groups_join_group' ) . '">' . __( 'Join Group', 'bp_magazine' ) . '</a>';
			} else if ( 'private' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="request-membership" rel="join" title="' . __( 'Request Membership', 'bp_magazine' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'request-membership', 'groups_send_membership_request' ) . '">' . __( 'Request Membership', 'bp_magazine' ) . '</a>';
			}
		}
	}
}
add_action( 'wp_ajax_joinleave_group', 'bp_dtheme_ajax_joinleave_group_magazine' );

/* AJAX close and keep closed site wide notices from an admin in the sidebar */
function bp_dtheme_ajax_close_notice_magazine() {
	global $userdata;

	if ( !isset( $_POST['notice_id'] ) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem closing the notice.', 'bp_magazine') . '</p></div>';
	} else {
...
add_action( 'wp_ajax_messages_close_notice', 'bp_dtheme_ajax_close_notice_magazine' );

/* AJAX send a private message reply to a thread */
function bp_dtheme_ajax_messages_send_reply_magazine() {
	global $bp;
	...
	if ( $result ) { ?>
		<div class="message-box new-message">
			<div class="message-metadata">
				...
				<strong><a href="<?php echo $bp->loggedin_user->domain ?>"><?php echo $bp->loggedin_user->fullname ?></a> <span class="activity"><?php printf( __( 'Sent %s', 'bp_magazine' ), bp_core_time_since( bp_core_current_time() ) ) ?></span></strong>
	...
	<?php
		...
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem sending that reply. Please try again.', 'bp_magazine' ) . '</p></div>';
	}
}
add_action( 'wp_ajax_messages_send_reply', 'bp_dtheme_ajax_messages_send_reply_magazine' );

/* AJAX mark a private message as unread in your inbox */
function bp_dtheme_ajax_message_markunread_magazine() {
	global $bp;

	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem marking messages as unread.', 'bp_magazine' ) . '</p></div>';
	} else {
...
add_action( 'wp_ajax_messages_markunread', 'bp_dtheme_ajax_message_markunread_magazine' );

/* AJAX mark a private message as read in your inbox */
function bp_dtheme_ajax_message_markread_magazine() {
	global $bp;

	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem marking messages as read.', 'bp_magazine' ) . '</p></div>';
	} else {
		$thread_ids = explode( ',', $_POST['thread_ids'] );
...
add_action( 'wp_ajax_messages_markread', 'bp_dtheme_ajax_message_markread_magazine' );

/* AJAX delete a private message or array of messages in your inbox */
function bp_dtheme_ajax_messages_delete_magazine() {
	global $bp;

	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem deleting messages.', 'bp_magazine' ) . '</p></div>';
	} else {
		$thread_ids = explode( ',', $_POST['thread_ids'] );
		...
		_e( 'Messages deleted.', 'bp_magazine' );
	}
}
add_action( 'wp_ajax_messages_delete', 'bp_dtheme_ajax_messages_delete_magazine' );

/**
 * bp_dtheme_ajax_messages_autocomplete_results_magazine()
 *
 ...
 * @return none
 */
function bp_dtheme_ajax_messages_autocomplete_results_magazine() {
	global $bp;
...
add_action( 'wp_ajax_messages_autocomplete_results', 'bp_dtheme_ajax_messages_autocomplete_results_magazine' );

...
<div id="fep-posts">
  <div id="fep-message"></div>
	<ul>
		<li><a <?php if($status == 'publish'): ?>class="active"<?php endif; ?> href="?fep_type=publish">Live</a></li>
		<li><a <?php if($status == 'pending'): ?>class="active"<?php endif; ?> href="?fep_type=pending">Pending</a></li>
	</ul>
	<div id="fep-post-table-container">
		<?php if( !$author_posts->have_posts() ): ?> 
			You don't have any <?=(($status=='publish')?'live':$status)?> articles.
		<?php else: ?>
			<p>You have <b class="count"><?=$author_posts->found_posts?></b> <?=(($status=='publish')?'live':$status)?> article(s).</p>
		<?php endif; ?>
		<table>
			<?php
			...
			<tr id="fep-row-<?=$postid?>" class="fep-row">
				<td><?php the_title(); ?></td>
				<?php if($status=='publish'): ?><td class="fep-fixed-td"><a href="<?php the_permalink(); ?>" title="View Post">View</a></td><?php endif; ?>
		...
		<div class="fep-nav">
			<?php if($new_exist): ?>
				<a class="fep-nav-link fep-nav-link-left" href="?fep_type=<?=$status?>&fep_page=<?=($paged-1)?>">&#10094; Newer Posts</a>
			<?php endif; ?>
			<?php if($old_exist): ?>
				<a class="fep-nav-link fep-nav-link-right" href="?fep_type=<?=$status?>&fep_page=<?=($paged+1)?>">Older Posts &#10095;</a>
			<?php endif; ?>

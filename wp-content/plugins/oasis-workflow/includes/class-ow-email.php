...
class OW_Email {
   ...
   public function post_published_notification( $new_status, $old_status, $post ) {
      ...
      if ( $new_status == 'publish' &&
           $old_status != 'publish' &&
            ...
            $subject =  $blog_name . __( "Your article has been published.", "oasisworkflow" );
            $user = get_userdata( $post->post_author );
            $to = $user->user_email;

            $msg = sprintf( __( '<div>Hello %1$s,</div><p>Your article <a href="%2$s" title="%3$s">%3$s</a> has been published on <a href="%4$s" title="%5$s">%5$s</a></p><p>Thanks.</p>', 'oasisworkflow' ), $user->display_name, esc_url( get_permalink( $post->ID ) ), $post->post_title, esc_url(get_bloginfo('url')),  get_bloginfo('name'));
   ...
   public function get_step_comment_content( $action_id ) {
      ...
      foreach ( $comments as $comment ) {
         ...
         if ( ! empty( $due_date ) ) {
            ...
            $due_date_text = ! empty( $workflow_terminology_options['dueDateText'] ) ? $workflow_terminology_options['dueDateText'] : __( 'Due Date', 'oasisworkflow' );
   ...
   public function get_user_sign_off_comments( $action_history_id, $comment_object, $post_id ) {
      ...
      {
         $comments_str .= "<p><strong>" . __('Additionally,', "oasisworkflow") . "</strong> {$full_name} " . __('added the following comments', "oasisworkflow") . ":</p>" ;
   ...
   public function notify_users_on_unauthorized_update( $assignees, $current_user_id, $post_id ) {
      ...
      {
         $blog_name = '[' . addslashes( get_bloginfo( 'name' )) . '] ';
         $subject =  $blog_name . __("Article was updated outside the workflow.", "oasisworkflow");
         foreach ($assignees as $assignee) {
            $user = get_userdata( $assignee );
            $post_info = get_post( $post_id );
            $to = $user->user_email;
            $msg = "<div>" . __("Hello ", "oasisworkflow");
            $msg = $msg . OW_Utility::instance()->get_user_name( $assignee ) . ",";
            $msg = $msg . "</div>";
            $msg = $msg . "<p>";
            $msg = $msg . OW_Utility::instance()->get_user_name( $current_user_id ) ;
            $msg = $msg . __(", who is not part of the current assignee list has updated the article ", "oasisworkflow");
            $msg = $msg . "<a href=" . esc_url( get_permalink( $post_id ) ) . " title=" . $post_info->post_title . ">" . $post_info->post_title . "</a>";
            $msg = $msg . __(" outside the workflow.", "oasisworkflow");
            $msg = $msg . "</p>";
   ...
   public function post_submit_notification( $post_id, $new_action_history_id ) {
      ...
      if ( isset( $email_settings['submit_to_workflow_email'] ) && $email_settings['submit_to_workflow_email'] == "yes" ) {
         ...
         $blog_name = '[' . addslashes( get_bloginfo( 'name' )) . '] ';
         $subject =  $blog_name . __( "Your article has been submitted.", "oasisworkflow" );
         $user = get_userdata( $post->post_author );
         $to = $user->user_email;

         $msg = sprintf( __( '<div>Hello %1$s,</div><p>Your article <a href="%2$s" title="%3$s">%3$s</a> has been successfully submitted to the workflow.</p><p>Thanks.</p>', 'oasisworkflow' ), $user->display_name, esc_url( get_permalink( $post_id ) ), $post->post_title, esc_url(get_bloginfo('url')),  get_bloginfo('name'));
         $message = '<html><head></head><body><div class="email_notification_body">'.$msg.'</div></body></html>';
   ...
   public function send_abort_email_to_author( $post_id ) {
      // sanitize the input
      $post_id = intval( $post_id );
      ...
      if ( isset( $email_settings['abort_email_to_author'] ) && $email_settings['abort_email_to_author'] == 'yes' ) {
         ...
         $blog_name = '[' . addslashes( get_bloginfo( 'name' ) ) . '] ';
         $subject = $blog_name . __( 'Workflow has been aborted.', "oasisworkflow" );
         $message = "<p>";
         $message .= __( "Hello ", "oasisworkflow" );
         $message .= $author->display_name . ",";
         $message .= "</p>";
         $message .= "<p>" . __( "Your article ", "oasisworkflow" );
         $message .= "<a href=" . esc_url( get_permalink( $post_id ) ) . " title=" . $post->post_title . ">" . $post->post_title . "</a>";
         $message .= __( " has been aborted from the workflow.", "oasisworkflow" );
         $message .= "</p>";
         $message .= "<p>";
         $message .= __( "If you have further questions regarding your article, please contact the administrator.", "oasisworkflow");
         $message .= "</p>";
